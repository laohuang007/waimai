<template>
    <div class="admin">
                ...
                <el-table :data="pager.lists" size="large">
                    ...
                    <el-table-column label="状态" min-width="100" v-perms="['auth.admin/edit']">
                        <template #default="{ row }">
                            <el-switch
                                ...
                                :inactive-value="1"
                                :disabled="row.root == 1"
                                @change="row.root != 1?changeStatus(row):''"
                            />
                        </template>
                    </el-table-column>
                    <el-table-column label="操作" width="120" fixed="right">
                        <template #default="{ row }">
                            ...
                            <el-button
                                v-perms="['auth.admin/money']"
                                type="primary"
                                link
                                @click="handleMoney(row)"
                            >
                                余额
                            </el-button>
                            <el-button
        ...
        <money-popup v-if="showMoney" ref="moneyRef" @success="getLists" @close="showMoney = false" />
    </div>
</template>
...
const moneyRef = shallowRef<InstanceType<typeof MoneyPopup>>()
// 表单数据
...
const handleMoney = async (data: any) => {
    showMoney.value = true
    await nextTick()
    moneyRef.value?.open()
    moneyRef.value?.setFormData(data)
}
